...
DrawingBoard.Control.Upload = DrawingBoard.Control.extend({
	...
	initialize: function() {
		this.$el.append('<label class=""><span class="glyphicon glyphicon-folder-open" style="font-size:9px;" aria-hidden="true"></span><input type="file" name="drawing_board_image_upload" style="display:none"/></label>');
		this.$el.on('change', 'input[name="drawing_board_image_upload"]', $.proxy(function(e) {
			var url = URL.createObjectURL(e.target.files[0]);
			this.board.setImg(url);
			e.preventDefault();
		}, this));
	}
});